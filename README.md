...
4. Create training dataset from current messages in your mail account (you can specify number of messages, default is 100):
  ...
  ```
5. In the next step you will be training binary classifier, so tag messages either with 0 or 1 (1 means that message should have new label).
...
6. Train classifier on tagged data:
  ...
  ```
7. Classify the newest received email message in your inbox:
  ```sh
